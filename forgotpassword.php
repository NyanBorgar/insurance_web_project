...
{
	$sql = "SELECT * FROM customer WHERE login_id='$_POST[loginid]' ";
	$var=mysqli_query($con,$sql);	
	if(mysqli_num_rows($var)==1)
	{
		$rs = mysqli_fetch_array($var);
		echo $rs[email_id];
	 	$msg = "Kindly recover password by entering clicking following link: http://localhost/insurpolicy/recoverpassword.php?id=".$rs[0];
